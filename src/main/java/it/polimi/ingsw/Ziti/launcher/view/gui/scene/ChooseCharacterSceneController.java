 ...
 */
/*
public class ChooseCharacterSceneController extends InputObservable implements GenericSceneController {
    ...
    public ChooseCharacterSceneController() {
        characterIndex = 0;
    ...
    public void initialize() {
        setFocusGodImage(Integer.toString(characters.get(0).getId()));
...
/*
    ...
    private void setFocusGodImage() {
        setFocusGodImage(Integer.toString(characters.get(characterIndex).getId()));
    }
    ...
    private void onNextGodBtnClick(Event event) {
        ...
        checkAndDisableButton(nextGodBtn, characters.size() - 1);
        checkSelectButtonsStatus();
    ...
    private void onSelectGodBtnClick(Event event) {
        selectedCharacter=(characters.get(characterIndex));
    ...
    private void onConfirmBtnClick(Event event) {
        ...
        if (selectedCharacter.size() < numberRequest) {
            SceneController.showAlert("Error", "Select exactly " + numberRequest + (numberRequest == 1 ? " god!" : " gods!"));
        } else {
            disableAllButtons();
            new Thread(() -> notifyObserver(obs -> obs.onUpdateGod(selectedGods))).start();
        }
    }
